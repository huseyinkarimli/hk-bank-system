        ...
        <div className="space-y-3">
          {transactions.map((transaction, index) => (
            <motion.div
              ...
              className="flex items-center justify-between p-4 rounded-lg bg-slate-800/30 hover:bg-slate-800/50 transition-colors duration-200"
            >
              ...
              <div className="text-right shrink-0 pl-2">
                ...
                >
                  {transaction.type === 'debit' ? '-' : '+'}
                  {transaction.amount.toLocaleString('en-HK')} {transaction.currency}
                </div>
              </div>
            </motion.div>
          ))}
